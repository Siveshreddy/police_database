...
st.dataframe(latest_logs_df, width='stretch')


def _running_via_streamlit() -> bool:

    import os
    if os.environ.get("STREAMLIT_SERVER_RUNNING"):
        return True
    import sys
    return any('streamlit' in (arg or '').lower() for arg in sys.argv)


if __name__ == '__main__':
    if not _running_via_streamlit():
        print("This file is a Streamlit app. To run it in a browser, use:\n\n    streamlit run d:/police project/app.py\n")
    else:
